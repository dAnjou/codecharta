...
export function calculateBuildingArea(area: number, smallestDelta: number, minimumBuildingArea: number) {
	if (area === 0) {
		return 0
	}
...
export function getBuildingAreas(childrenAreaValues: number[], smallestDelta: number, minimumBuildingSize: number) {
	return childrenAreaValues.map(element => {
		return calculateBuildingArea(element, smallestDelta, minimumBuildingSize)
	})
}
...
function getSquarifiedTreeMap(map: CodeMapNode, state: State): SquarifiedTreeMap {
	...
	const hierarchyNode = hierarchy(map)
	const padding = INITIAL_PADDING * (state.dynamicSettings.margin / 100)
